...
#Criar um Histograma para representar as frequências das classes
hist(natal$temperatura_c)

#A literatura sugere que o número de quebras em um histograma seja a Raiz quadrada das observações
sqrt(6938)
summary(natal)
#Histograma melhor apresentado, observe como cada argumento alterará algum componente do gráfico
hist(natal$temperatura_c, main = "Histograma Temperatura",xlab = "Temperatura",ylab="Frequência", col = "blue",breaks=83)
abline(v=26.50,lwd=3, col="green")
abline(v=26.39,lwd=3,col="orange")
abline(v=26.9,lwd=5,col="black")
legend("topright",abline,lwd=3, col=c("green","orange","black"),legend=c("Média","Mediana","Moda"))

#Calcular Variância
var(natal$temperatura_c)

#Calcular Desvio Padrão (raiz quadrada da variância)

sqrt(var(natal$temperatura_c))

#Ou de forma mais direta
sd(natal$temperatura_c)


#6. Medidas de Relação e Modelos básicos--------------------------------------------------
...
#Informaçõeses do Modelo
summary(modelo)
plot(natal$umidade_relativa_percentual, natal$temperatura_c)

#Regressão Linear Multipla
modelom=lm(natal$pm25_ugm3 ~ natal$temperatura_c + natal$precipitacao_mmdia + natal$umidade_relativa_percentual + natal$vento_velocidade_ms)
summary(modelom)

plot(modelom)

#7. Plotagens de Gráficos Simples--------------------------------------------------------
#Gráficos de série temporais simples
chuva=ts(data2[,4:5], start=c(2000,1), end=c(2018,12), frequency=12)
plot(chuva,xlab="Tempo",main="Série Temporal")

##Plotagens de caixas ou Boxplots
boxplot(natal$precipitacao_mmdia ~ natal$Mes,outline=F, main = "Boxplot Precipitação",ylab="Precipitação (mm)", xlab="Mês")


#8. Alguns indices de precipitação -> Usaremos o pacote Precintcon--------------------------------
...
#9. Alguns indices de temperatura -> Usaremos o pacote Thermindex--------------------------------
...
#10. Alguns indices de -> Usaremos o pacote ClimInd--------------------------------
...
#11. Possibilidades com o Pacote OpenAir
